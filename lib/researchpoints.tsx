...
export default function ResearchPoints(){
    ...
    const researchpoints=async ()=> {

                      setLoading(true);
  try {
    const request = await fetch("https://junaidb-askdocs.hf.space/researchproblems", {
      method: "POST",
      mode: "cors",
      body: JSON.stringify({
        owner: publicKey?.toBase58(),
  ...
  useEffect(() => {
   ...
   if (isGateEnabled && !token){
    ...
    <h3 className="text-xl font-semibold text-white mb-4">Key Research Problems</h3>
